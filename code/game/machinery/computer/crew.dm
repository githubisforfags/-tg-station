...
/datum/crewmonitor/proc/generateMiniMap(z, x1 = 1, y1 = 1, x2 = world.maxx, y2 = world.maxy)
	...
	for (var/turf/tile in tiles)
		if      (istype(tile.loc, /area/asteroid) || istype(tile, /turf/simulated/mineral) || (istype(tile.loc, /area/space) && istype(tile, /turf/simulated/floor/plating/asteroid || istype(tile.loc, /area/space/mine/unexplored))))
			temp = "/area/asteroid"
