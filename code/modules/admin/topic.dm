...
*/
		...
		switch(href_list["jobban3"])
			...
			if("nonhumandept")
				for(var/jobPos in nonhuman_positions)
					...
					joblist += temp.title
			if("ghostroles")
				joblist += list("pAI", "posibrain", "deathsquad")
			else
	...
	//Watchlist
		...
		usr.client.cmd_admin_mommize(H)
...
*/
			...
			if("hellonearth")
				...
				var/choice = input("You sure you want to end the round and summon narsie at your location?") in list("PRAISE SATAN", "Cancel")
			...
			if("supermattercascade")
				...
				var/choice = input("You sure you want to destroy the universe and create a large explosion at your location?") in list("NO TIME TO EXPLAIN", "Cancel")
